 ...
 **결과**  :목표 속도가 0으로 전환되는 정지 조건에서는 누적된 적분 오차가 이후 제어 출력에 영향을 주지 않도록 err_I를 초기화했습니다. 또한 PID 출력 범위를 제한하고, 미분항 수식을 현재 오차 - 이전 오차 기준으로 수정했습니다. 이후 로그값을 확인하며 Pv = 4.0, Iv = 0.0155, Dv = 0.5로 게인을 조정했고, 목표 속도 변화에 따라 모터가 급격한 튐 없이 안정적으로 응답하도록 개선했습니다.
